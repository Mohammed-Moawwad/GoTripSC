...
class AdminDashboard {
  ...
  handleServiceClick(card) {
    ...
    if (!this.currentAdmin.permissions.includes(service)) {
      this.showNotification(
        `You don't have permission to manage ${this.services[service].name}`,
        "error"
      );
      return;
    }
    ...
    this.showNotification(
      `Opening ${this.services[service].name} management...`,
      "info"
    );
  ...
  simulateServiceLoading(service) {
    ...
    setTimeout(() => {
      ...
      this.showNotification(
        `${this.services[service].name} management loaded successfully!`,
        "success"
      );
    }, 1500);
  }
    ...
    setTimeout(() => {
      const modalContent = `
                <div class="profile-modal">
                    ...
                    <p><strong>Permissions:</strong> ${this.currentAdmin.permissions.join(
                      ", "
                    )}</p>
