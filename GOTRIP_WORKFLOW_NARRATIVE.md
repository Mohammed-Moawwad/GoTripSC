...
## 9. My Bookings & Cancellation - Manage My Bookings

The My Bookings page (`User/MyBookings.html`) shows a list of all the user's bookings. Each booking shows the booking ID, type (hotel/flight/bus), destination, dates, price, and status. Users can search or filter bookings by date or status. Each booking has two buttons: one to see full details and one to cancel. When users click cancel, a confirmation message appears. The system sends `PUT /api/bookings/{bookingId}/cancel` to cancel the booking and saves this change in the database.

---

## 10. My Trips & Booking History - See My Trips

The My Trips page (`User/my-trips.html`) shows bookings organized by type (flights, hotels, buses) or by date (future trips and past trips). Future trips are shown in a special way so users can see them easily. Past trips are saved but not highlighted. Users can click on any trip to see more information like booking number, payment details, hotel/airline phone number, and special notes. This helps users remember their travel plans even after trips are finished.
...
## 12. Hotel Management (Admin) - Add & Edit Hotels

Hotel Management (`Admin/AdminHotels/`) is where admins see all hotels. Each hotel row shows ID, name, city, country, star rating, price per night, number of rooms, extras (amenities), and status (active or not). Admins can add new hotels by filling in a form with name, city, address, phone, email, description, price, room count, and extras. When they save, the system creates the new hotel using `POST /api/hotels`. Admins can also change hotel information or hide hotels from the website.

---

## 13. Flight Management (Admin) - Add & Edit Flights

Flight Management (`Admin/AdminFlights/`) shows all flights on the website. Each flight shows ID, airline name, plane type, start and end cities, times, price, available seats, and status. Admins add new flights by filling in a form with airline name, flight number, plane type, cities, times, price, and seats. When they save, the system creates the flight using `POST /api/flights`. Admins can also change flight information. When they delete a flight, it is removed from the system.

---

## 14. Bus Management (Admin) - Add & Edit Buses

Bus Management (`Admin/AdminBus/`) shows all buses on the website. Each bus shows ID, bus company name, bus type, start and end cities, times, price, seats, and status. Admins add new buses by filling in a form with company name, bus number, bus type (standard, deluxe, or AC), cities, times, price per seat, number of seats, and extras. When they save, the system creates the bus using `POST /api/buses`. Admins can change bus information or delete buses when they are no longer used.
