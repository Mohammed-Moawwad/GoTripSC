...
function arabicIndicToWestern(s) {
  return String(s || "").replace(/[٠-٩]/g, (d) =>
...
function generateFakeTripsForDate({
  dateYMD,
  from,
  to,
  N = 20,
  ...
  for (let i = 0; i < N; i++) {
    ...
    const companyIndex = deterministic
      ? (i + rnd(0, companies.length - 1)) % companies.length
      : randInt(0, companies.length - 1);
    ...
    const arrTime = `${String(arrDate.getHours()).padStart(2, "0")}:${String(
      arrDate.getMinutes()
    ).padStart(2, "0")}`;
    ...
    trips.push({
      ...
      operator: company,
      operatorLogo: `https://placehold.co/80x80/${
        [
          "FF6B6B",
          "4ECDC4",
          "FFE66D",
...
async function loadMockData(searchParams = {}) {
  try {
    ...
    try {
      ...
      console.log("No mock_buses.json found, using generated data");
    }
...
function searchTrips(searchParams) {
  ...
  if (searchParams.class) {
    results = results.filter((bus) => bus.class === searchParams.class);
  }
  ...
  if (searchParams.minPrice) {
    results = results.filter(
      (bus) => bus.price >= parseFloat(searchParams.minPrice)
    );
  }
  if (searchParams.maxPrice) {
    results = results.filter(
      (bus) => bus.price <= parseFloat(searchParams.maxPrice)
    );
  }
  ...
  if (searchParams.operators && searchParams.operators.length > 0) {
    results = results.filter((bus) =>
      searchParams.operators.includes(bus.operator)
    );
  }
...
function getBestTags(results) {
  ...
  const cheapest = results.reduce(
    (min, bus) => (bus.price < min.price ? bus : min),
    results[0]
  );
...
function renderResults(results) {
  ...
  // Render cards
  listContainer.innerHTML = pageResults
    .map((bus) => {
      const departTime = formatTime(bus.departure);
      const arriveTime = formatTime(bus.arrival);
      const duration = formatDuration(
        bus.duration || bus.duration_minutes || 90
      );
      const stopsText =
        bus.stops === 0
          ? strings[lang].nonstop
          : bus.stops === 1
          ? `1 ${strings[lang].stop}`
      ...
      if (currentSort === "recommended" && bus.id === tags.recommended) {
        badgeHtml =
          '<span class="tag-badge tag-recommended">Recommended</span>';
      ...
      <article class="result-card" data-id="${bus.id}">
        <div class="operator-section">
          <img src="${bus.operatorLogo || bus.operator_logo}" alt="${
        bus.operator
      }" class="operator-logo" />
        ...
        <div class="price-section">
          <div class="price-amount">
            ...
            <div class="price-value">
              ${bus.price.toFixed(2)} <span class="price-currency">${
        bus.currency || "SAR"
      }</span>
            </div>
          </div>
          <button class="select-btn" onclick="handleSelect('${bus.id}')">${
        strings[lang].select
      }</button>
        </div>
      </article>
  ...
  const passengers = `${params.passengers} Adult${
    params.passengers !== "1" ? "s" : ""
  }`;
  document.getElementById("resultsTitle").textContent = `${
    strings[lang].results
  }: ${results.length} ${strings[lang].trips}${route ? " — " + route : ""}${
    date ? " — " + date : ""
          ...
          <div class="trip-info-section">
            ...
            <div class="info-row">
              ...
              <span class="info-value ${
                (bus.seatsAvailable || bus.seats_left) <= 5
                  ? "text-danger"
                  : "text-success"
              }">
            ...
            <div class="info-row">
              ...
              <span class="info-value price-large">${bus.price.toFixed(2)} ${
    bus.currency || "SAR"
  }</span>
            </div>
          </div>
        ...
        <div class="modal-footer">
          ...
          <button class="btn-primary" onclick="proceedToBooking('${
            bus.id
...
function proceedToBooking(busId) {
  ...
  if (!isLoggedIn) {
    ...
    const returnUrl = encodeURIComponent(
      `/Services/Buses/Booking.html?${bookingParams.toString()}`
    );
    window.location.href = `../../Login/login.html?next=${returnUrl}`;
  } else {
    // Go to booking page
...
function applyFiltersFromUI() {
  ...
  const operators = Array.from(
    document.querySelectorAll('input[name="operator"]:checked')
  ).map((cb) => cb.value);
...
function setupEventListeners() {
  ...
  // Apply filters automatically when any filter input changes
  document.querySelectorAll('input[name="stops"]').forEach((checkbox) => {
    checkbox.addEventListener("change", applyFiltersFromUI);
  });

  document.querySelectorAll('input[name="operator"]').forEach((checkbox) => {
    checkbox.addEventListener("change", applyFiltersFromUI);
  });
  ...
  document
    ...
    ?.addEventListener("input", applyFiltersFromUI);
  document
  ...
  document.getElementById("resetFilters").addEventListener("click", () => {
    ...
    document
      .querySelectorAll('input[name="operator"]')
      .forEach((cb) => (cb.checked = false));
    document
  ...
  currencyBtn?.addEventListener("click", (e) => {
    ...
    const currentCurrency = currencyBtn.textContent
      .trim()
      .split("\n")[0]
      .trim();
